...
public:
    std::vector<std::vector<int>> highestPeak(std::vector<std::vector<int>>& isWater) {
        int ROWS = static_cast<int>(isWater.size());
        int COLS = static_cast<int>(isWater[0].size());
        
        std::vector<std::vector<int>> heights(ROWS, std::vector<int>(COLS, -1)); 
        ...
        while (!q.empty()) {
            ...
            int current_height = heights[r][c];
